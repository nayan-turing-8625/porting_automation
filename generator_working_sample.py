...
# Live Google Sheet ("Working Sheet") → Colab notebooks in Drive + summary tab
# - Query populated from column 'query'
# - Final Assertion code populated from 'final_assertion_code'
# - Initial Assertion & Action remain empty
# - Import DB section unchanged (live porting code from 'Translate_JSONs', select services by *_initial_db columns)
# - Notebook name: Gemini_Apps_Data_Port_<task_id>_turn<query_order>.ipynb
# - Summary tab (default 'Generated_Colabs_Working'): task_id, query_order, colab_url, refresh_date, refresh_time (Pacific)
...
SOURCE_SHEET_NAME    = os.environ.get("SOURCE_WORKING_SHEET_NAME", "Working Sheet").strip()  # default to "Working Sheet"
...
def upsert_summary_sheet_working(sheets, spreadsheet_id: str, sheet_name: str, rows: List[List[str]]):
    """
    Writes FIVE columns: task_id, query_order, colab_url, refresh_date, refresh_time
    into a sheet named `sheet_name` (created if missing, cleared if exists).
    """
    ...
    rows_5 = [r + [refresh_date, refresh_time] for r in rows]
    headers = ["task_id", "query_order", "colab_url", "refresh_date", "refresh_time"]
    body = {"range": f"'{sheet_name}'!A1", "majorDimension": "ROWS", "values": [headers] + rows_5}
...
def generate_notebook_for_row(
    ...
    # Assertions & action
    nb.cells.extend(build_empty_block("Initial Assertion"))   # empty
    nb.cells.extend(build_empty_block("Action"))              # empty
    nb.cells.extend(build_final_assertion_block(row))         # populated from 'final_assertion_code'
...
def main():
    ...
    # Resolve source task sheet (Working Sheet by default)
    ...
    for i, row in enumerate(task_rows, start=1):
        ...
        log.info("---- Generating notebook for task_id=%s (turn=%s) ----", task_id, query_order)
        ...
        rows_summary.append([task_id, query_order, colab_url])
    ...
    # Write summary tab back to spreadsheet (task_id, query_order, colab_url, refresh_date/time)
    if SUMMARY_SHEET_NAME:
        upsert_summary_sheet_working(sheets, SPREADSHEET_ID, SUMMARY_SHEET_NAME, rows_summary)
        log.info("Wrote summary tab '%s' with %d rows.", SUMMARY_SHEET_NAME, len(rows_summary))

    log.info("Completed generation for %d row(s).", len(rows_summary))
