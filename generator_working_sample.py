...
#
# NEW (fix):
#  • Action block now starts with imports for the UNION of initial-services (from *_initial_db columns)
#    and final-services (from 'final_state_changes_needed'), including their dependencies.
#
# PREVIOUS FIXES PRESERVED:
#  • Initial DB selection is DATA-DRIVEN by scanning all <service>_initial_db columns.
#  • Action block applies FINAL DB using the SAME porting calls as initial stage.
#  • WhatsApp final stage: use contacts_final_db if present; else fall back to contacts_initial_db (Template).
...
def build_import_and_port_cell_ws(
    ...
    single_line = " ".join(raw_loc.split())  # collapses whitespace/newlines
...
def build_action_final_dbs_cell_ws(
    ...
    template_row: Dict[str, str],
    action_api_modules: List[str],   # <-- NEW: pass in the union of APIs needed
):
    """
    Builds the Action block code cell that applies FINAL DB changes.
    Starts with an Imports section based on 'action_api_modules'.
    """
    L: List[str] = []

    # --- Imports for Action block ---
    L.append("# Imports (Action)")
    if action_api_modules:
        ...
        if "notes_and_lists" in action_api_modules:
            ...
            L.append("from datetime import timezone")
    L += ["import json, uuid", "from datetime import datetime", "import os", ""]
    # (No reload of default DBs or USER_LOCATION here.)
...
def generate_notebook_for_row_ws(
    ...
    # Compute union for Action imports
    all_services_for_action = list(dict.fromkeys(services_for_code + final_services))
    action_api_modules = api_modules_for_services(all_services_for_action)
    ...
    nb.cells.append(
        build_action_final_dbs_cell_ws(
            ...
            template_row=template_row,
            action_api_modules=action_api_modules,  # <-- ensure imports appear
        )
    )
